...
def sync_do_slice(
    ...
    for slice_number in range(slices):
        ...
        # Only pad shorter columns when some slices received an extra item.
        # When length is divisible by slices, every column is already full.
        target_len = items_per_slice + (1 if slices_with_extra else 0)
        if fill_with is not None and len(tmp) < target_len:
